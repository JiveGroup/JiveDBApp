...
SELECT strftime('%Y-%m', created_at) AS month, count(*) AS rows FROM core_user GROUP BY month ORDER BY month DESC;
...
--  ROW_NUMBER, emulate ROLLUP bằng UNION ALL, conditional aggregation,
--  JSON lồng, cohort). Dùng nhóm bảng core_* có quan hệ: core_user → core_order,
--  core_user → core_item → core_event(amount), core_tag → core_note.
--  Chỉ ĐỌC — an toàn. Đã kiểm chứng chạy trên jdb_medium.sqlite.
-- ============================================================================

-- 8) CTE ĐỆ QUY: tạo dải tháng (lịch) + đếm đơn theo tháng, lấp tháng trống
WITH RECURSIVE bounds AS (
  SELECT strftime('%Y-%m-01', MIN(created_at)) AS lo, strftime('%Y-%m-01', MAX(created_at)) AS hi FROM core_order
),
months AS (
  SELECT lo AS m FROM bounds
  UNION ALL
  SELECT date(m, '+1 month') FROM months, bounds WHERE m < bounds.hi
)
SELECT strftime('%Y-%m', m) AS month, COUNT(o.id) AS orders
FROM months
LEFT JOIN core_order o ON strftime('%Y-%m-01', o.created_at) = m
GROUP BY m
ORDER BY m;
...
WITH monthly AS (
  SELECT strftime('%Y-%m', created_at) AS month, SUM(amount) AS revenue, COUNT(*) AS events
  FROM core_event GROUP BY strftime('%Y-%m', created_at)
)
...
-- 12) ROW_NUMBER (thay LATERAL): mỗi user → đơn gần nhất + core_item doanh thu cao nhất
WITH last_order AS (
...
SELECT cu.full_name AS user, lo.id AS last_order_id, lo.status AS last_status, lo.created_at AS last_at,
...
-- 13) ROLLUP (emulate bằng UNION ALL): số đơn theo country × status + dòng tổng
WITH s AS (SELECT COALESCE(country, '?') AS country, COALESCE(status, '?') AS status, id FROM core_order)
SELECT country, status, COUNT(*) AS orders FROM s GROUP BY country, status
UNION ALL
SELECT country, '(all statuses)', COUNT(*) FROM s GROUP BY country
UNION ALL
SELECT '(all countries)', '(all statuses)', COUNT(*) FROM s
ORDER BY country, status;

-- 14) COHORT: theo tháng đặt đơn ĐẦU TIÊN, số user còn hoạt động theo offset tháng
WITH first_order AS (
  SELECT core_user_id, strftime('%Y-%m', MIN(created_at)) AS cohort
  FROM core_order GROUP BY core_user_id
),
activity AS (
  SELECT o.core_user_id, fo.cohort, strftime('%Y-%m', o.created_at) AS month
...
-- 16) JSON LỒNG: mỗi user kèm mảng đơn + số item (json_group_array/json_object)
SELECT json_group_array(json_object(
         'user', cu.full_name, 'status', cu.status,
         'orders', json((SELECT json_group_array(json_object('id', o.id, 'status', o.status, 'city', o.city))
